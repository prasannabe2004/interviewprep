 ...
 *
 * This file implements a thread-safe circular queue (ring buffer) specifically
 * designed for handling network packets or data packets in a producer-consumer
 * scenario.
 *
 * Key Features:
 ...
 * - Any scenario requiring efficient FIFO data structure with overwrite
 * capability
 *
...
class PacketBuffer {
   private:
    ...
    mutable std::condition_variable
        cv;  // for immediate consumption notification
...
void consumer(PacketBuffer& buf) {
    for (int i = 0; i < 10; ++i) {
        // dequeue() now blocks until packet is available (immediate
        // consumption)
